...
class License {
    ...
    // TODO: Plan for EDD for User Menus.
	/**
	 * EDD API URL.
	 *
	 * @var string
	 */
	const API_URL = 'https://usermenusplugin.com/edd-sl-api/';
	 ...
	 * @var string
	 */
	const OPTION_KEY = 'user_menus_license';
	...
	public function register_hooks() {
		...
		add_action( 'user_menus_license_status_check', [ $this, 'refresh_license_status' ] );
	...
	public function autoregister() {
		if ( defined( '\USER_MENUS_LICENSE_KEY' ) && ! empty( \USER_MENUS_LICENSE_KEY ) && '' === $this->get_license_key() ) {
				$this->activate_license( \USER_MENUS_LICENSE_KEY );
		}
	}
	...
	public function schedule_crons() {
		if ( ! wp_next_scheduled( 'user_menus_license_status_check' ) ) {
			wp_schedule_event( time(), 'daily', 'user_menus_license_status_check' );
		}
	}
	...
	public function update_license_key( $key ) {
		 ...
		 */
		\do_action( 'user_menus_license_key_changed', $key, $old_key );
	}
	...
	public function update_license_status( $license_status ) {
		 ...
		 */
		\do_action( 'user_menus_license_status_updated', $license_status, $previous_status );
	}
	...
	private function check_license() {
		$api_params = [
			...
			'item_id'     => self::ID,
			'item_name'   => rawurlencode( 'User Menus Pro' ),
			'url'         => home_url(),
	...
	public function activate_license( $key = null ) {
		...
		$api_params = [
			...
			'license'     => $key,
			'item_id'     => self::ID,
			'item_name'   => rawurlencode( 'User Menus' ),
			'url'         => home_url(),
			'environment' => function_exists( 'wp_get_environment_type' ) ? wp_get_environment_type() : 'production',
		];
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			if ( is_wp_error( $response ) ) {
				...
				$message = __( 'An error occurred, please try again.', 'user-menus' );
			}
		...
		if ( $this->is_license_active() ) {
			if ( ! get_option( 'user_menus_pro_activation_date', false ) ) {
				update_option( 'user_menus_pro_activation_date', time() );
			}
		}
		...
		switch ( $license_status['error'] ) {
			case 'expired':
				$message = sprintf(
					...
					__( 'Your license key expired on %s.', 'user-menus' ),
			...
			case 'disabled':
			case 'revoked':
				$message = __( 'Your license key has been disabled.', 'user-menus' );
				break;

			case 'missing':
					$message = __( 'Invalid license.', 'user-menus' );
				break;

			case 'invalid':
			case 'site_inactive':
				$message = __( 'Your license is not active for this URL.', 'user-menus' );
				break;

			case 'item_name_mismatch':
				$message = sprintf(
					/* translators: the plugin name */
					__( 'This appears to be an invalid license key for %s.', 'user-menus' ),
					__( 'User Menus', 'user-menus' )
				);
				break;

			case 'no_activations_left':
				$message = __( 'Your license key has reached its activation limit.', 'user-menus' );
				break;
