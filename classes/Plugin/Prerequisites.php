...
class Prerequisites {
	...
	public function get_php_message( $failed_check_args ) {
		...
		$message = __( 'This plugin requires <b>%1$s %2$s</b> or higher in order to run.', 'user-menus' );
		return sprintf(
			$message,
			__( 'PHP', 'default' ),
	...
	public function get_plugin_message( $failed_check_args ) {
		...
		if ( isset( $failed_check_args['not_activated'] ) ) {
			...
			$link = '<a href="' . $url . '">' . __( 'activate it', 'user-menus' ) . '</a>';

			$text = sprintf(
				/* translators: 1. Plugin Name, 2. Required Plugin Name, 4. `activate it` link. */
				__( 'The plugin "%1$s" requires %2$s! Please %3$s to continue!', 'user-menus' ),
				$dep_label,
				'<strong>' . $name . '</strong>',
				$link
			);
			...
			$link = '<a href="' . $url . '">' . __( 'update it', 'user-menus' ) . '</a>';

			$text = sprintf(
				/* translators: 1. Plugin Name, 2. Required Plugin Name, 3. Version number, 4. `update it` link. */
				__( 'The plugin "%1$s" requires %2$s v%3$s or higher! Please %4$s to continue!', 'user-menus' ),
				$dep_label,
				'<strong>' . $name . '</strong>',
			...
			$link = '<a href="' . $url . '">' . __( 'install it', 'user-menus' ) . '</a>';
