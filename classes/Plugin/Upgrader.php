...
class Upgrader {
	...
	public function debug_log( $message, $type = 'INFO' ) {
		if ( defined( 'USER_MENUS_LOGGING' ) && USER_MENUS_LOGGING ) {
	...
	public function activate_plugin( $plugin_basename ) {
		if ( ! $plugin_basename || empty( $plugin_basename ) ) {
			return new \WP_Error( 'user_menus_plugin_basename', __( 'Plugin basename empty.', 'user-menus' ) );
		}
	...
	public function install_plugin( $file ) {
		...
		if ( false === $this->get_fs_creds() ) {
			return new \WP_Error( 'user_menus_fs_creds', __( 'Unable to get filesystem credentials.', 'user-menus' ) );
		}

		// Do not allow WordPress to search/download translations, as this will break JS output.
		remove_action( 'upgrader_process_complete', [ 'Language_Pack_Upgrader', 'async_upgrade' ], 20 );

		$installer = new \UserMenus\Installers\PluginSilentUpgrader( new \UserMenus\Installers\Install_Skin() );

		// 1. Check if the plugin exists already, if so upgrade it.

		// Error check.
		if ( ! method_exists( $installer, 'install' ) ) {
			return new \WP_Error( 'user_menus_upgrader', __( 'Upgrader missing install method.', 'user-menus' ) );
		}
