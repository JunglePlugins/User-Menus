...
class ObjectSearch extends WP_REST_Controller {
		...
		register_rest_route(
			...
			[
				[
					...
					'args'                => [
						'nonce'      => [
							'description' => __( 'Nonce', 'user-menus' ),
							'type'        => 'string',
							'required'    => true,
						],
						'hash'       => [
							'description' => __( 'Hash', 'user-menus' ),
							'type'        => 'string',
							'required'    => true,
						],
						'entityKind' => [
							'description' => __( 'Object kind, (postType, taxonomy)', 'user-menus' ),
							'type'        => 'string',
							'required'    => true,
						],
						'entityType' => [
							'description' => __( 'Object type (post, category)', 'user-menus' ),
							'type'        => 'string',
							'required'    => true,
						],
						'paged'      => [
							'description' => __( 'Page number', 'user-menus' ),
							'type'        => 'integer',
							'required'    => false,
						],
						's'          => [
							'description' => __( 'Search term', 'user-menus' ),
							'type'        => 'string',
							'required'    => false,
						],
						'include'    => [
							'description' => __( 'Include IDs', 'user-menus' ),
							'type'        => 'string',
							'required'    => false,
						],
						'exclude'    => [
							'description' => __( 'Exclude IDs', 'user-menus' ),
							'type'        => 'string',
							'required'    => false,
						],
					],
	...
	public function object_search( $request ) {
		...
		if ( ! isset( $nonce ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $nonce ) ), 'user_menus_object_search_nonce' ) ) {
			wp_send_json_error();
		}
		...
		if ( $results ) {
			...
			return new WP_Error( '404', __( 'Something went wrong, the results could not be returned.', 'user-menus' ), [ 'status' => 404 ] );
		}
	}
