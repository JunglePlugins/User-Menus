...
use function \UserMenus\remap_conditions_to_query;

/**
 * Restrictions v2 migration.
 */
class Restrictions_2 extends \UserMenus\Base\Upgrade {
	...
	public function label() {
		return __( 'Update global restrictions', 'user-menus' );
	}
	...
	public function run() {
		...
		if ( $count ) {
			$stream->start_task(
				...
				sprintf( __( 'Migrating %d restrictions', 'user-menus' ), $count ),
				$count
			);
				...
				if ( $this->migrate_restriction( $restriction ) ) {
					...
					$stream->send_event(
						...
						[
							...
							'message'     => sprintf( __( 'Failed to migrate restriction "%s".', 'user-menus' ), $restriction['title'] ),
		...
		if ( ! $count || $progress === $count ) {
			...
			$stream->complete_task( sprintf( __( '%d restrictions migrated', 'user-menus' ), $count ) );
		} else {
			$stream->send_event(
				'task:error',
				[
					// translators: %d: number of restrictions that failed to migrate.
					'message' => sprintf( __( 'Failed to migrate %d restrictions.', 'user-menus' ), count( $settings['restrictions'] ) ),
				]
			);
