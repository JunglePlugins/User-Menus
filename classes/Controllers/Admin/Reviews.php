...
use function UserMenus\get_option;
use function UserMenus\plugin;

defined( 'ABSPATH' ) || exit;

/**
 * Class UserMenus\Admin\Reviews
 *
...
class Reviews extends Controller {
	...
	public function init() {
		...
		add_action( 'wp_ajax_user_menus_review_action', [ $this, 'ajax_handler' ] );
	}
	...
	public function installed_on() {
		$installed_on = get_option( 'user_menus_installed_on', false );

		if ( ! $installed_on ) {
			$installed_on = current_time( 'mysql' );
			update_option( 'user_menus_installed_on', $installed_on );
		}
	...
	public function ajax_handler() {
		...
		if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( wp_unslash( $_REQUEST['nonce'] ), 'user_menus_review_action' ) ) {
			wp_send_json_error();
		}
		...
		try {
			...
			update_user_meta( $user_id, 'user_menus_reviews_dismissed_triggers', $dismissed_triggers );
			update_user_meta( $user_id, 'user_menus_reviews_last_dismissed', current_time( 'mysql' ) );
	...
	public function dismissed_triggers() {
		...
		$dismissed_triggers = get_user_meta( $user_id, 'user_menus_reviews_dismissed_triggers', true );
	...
	public function already_did( $set = false ) {
		...
		if ( $set ) {
			update_user_meta( $user_id, '_user_menus_reviews_already_did', true );

			return true;
		}

		return (bool) get_user_meta( $user_id, '_user_menus_reviews_already_did', true );
	}
	...
	public function triggers( $group = null, $code = null ) {
		...
		if ( ! isset( $triggers ) ) {
			...
			$time_message = __( 'Hi there! %1$s You\'ve been using the %2$sUser Menus%3$s plugin on your site for %4$s now - We hope it\'s been helpful. If you\'re enjoying the plugin, would you mind rating it %5$s5-stars%6$s to help spread the word?', 'user-menus' );
			$triggers     = [
				'time_installed' => [
					'triggers' => [
						'one_week'     => [
			...
			$triggers = apply_filters( 'user_menus_reviews_triggers', $triggers );
	...
	public function admin_notices() {
			...
			(function ($) {
				...
				function dismiss(reason) {
					$.ajax({
						...
						url: ajaxurl,
						data: {
							action: 'user_menus_review_action',
							nonce: '<?php echo esc_js( wp_create_nonce( 'user_menus_review_action' ) ); ?>',
							group: trigger.group,
							code: trigger.code,
							pri: trigger.pri,
							reason: reason
				...
				$(document)
					.on('click', '.user-menus-notice .user-menus-dismiss', function (event) {
						var $this = $(this),
							reason = $this.data('reason'),
							notice = $this.parents('.user-menus-notice');
					...
					.ready(function () {
						setTimeout(function () {
							$('.user-menus-notice button.notice-dismiss').click(function (event) {
								dismiss('maybe_later');
							});
						}, 1000);
					});
		...
		<style>
			...
			.user-menus-notice .review-actions {
				margin-top: 10px;
				margin-bottom: 0;
				padding-left: 0;
				list-style: none;
			...
			.user-menus-notice .review-actions li {

			}

			.user-menus-notice .five-stars {}

			.user-menus-notice .five-stars::before {
				content: "★★★★★";
				color: #f0ad4e;
			}
		</style>

		<div class="notice notice-success is-dismissible user-menus-notice">
			...
			<div class="notice-content">
				...
				<ul class="review-actions">
					<li>😁
						<a class="user-menus-dismiss" target="_blank" href="<?php echo esc_attr( $trigger['link'] ); ?>" data-reason="am_now">
							<strong><?php esc_html_e( 'Ok, you deserve it', 'user-menus' ); ?></strong>
						</a>
					</li>
					<li>
						🤔
						<a href="#" class="user-menus-dismiss" data-reason="maybe_later">
							<?php esc_html_e( 'Maybe later, okay?', 'user-menus' ); ?>
						</a>
					</li>
					<li>🙌
						<a href="#" class="user-menus-dismiss" data-reason="already_did">
							<?php esc_html_e( 'I already did', 'user-menus' ); ?>
						</a>
					</li>
				</ul>
	...
	public function last_dismissed() {
		...
		return get_user_meta( $user_id, 'user_menus_reviews_last_dismissed', true );
	}
