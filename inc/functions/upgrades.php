...
function current_data_versions() {
	return apply_filters( 'user_menus/current_data_versions', [
		'settings'     => 2,
		'restrictions' => 2,
		'plugin_meta'  => 2,
		'user_meta'    => 2,
...
function get_data_versions() {
	$versioning = get_option( 'user_menus_data_versioning', [] );
...
function set_data_version( $key, $version ) {
	...
	return update_option( 'user_menus_data_versioning', $versioning );
}
...
add_action( 'user_menus/update_version', __NAMESPACE__ . '\maybe_force_v2_migrations' );
...
function maybe_force_v2_migrations( $old_version ) {
	if ( version_compare( $old_version, '2.0.0', '<' ) ) {
		...
		update_option( 'user_menus_data_versioning', $versioning );
	}
}
